...
class _CommunityFeedScreenState extends State<CommunityFeedScreen> {
  ...
  bool posting = false;
  int selectedTab = 0;
  bool showingReels = false;
  int _mobileSocialCategory = 0;
  ...
  Future<void> _initialize() async {
    ...
    await Future.wait<void>([_syncCommunityProfile(), _loadFeed()]);
  }
  ...
  Future<void> _loadFeed({bool loadMore = false}) async {
    ...
    try {
      ...
      await Future.wait<void>(
        (memberRows as List).map((raw) async {
      ...
      final mappedPostRows = (postRows as List)
      ...
      final socialResults = await Future.wait<dynamic>([
        ...
        memberIds.isEmpty
            ...
            : Supabase.instance.client
                  .from('community_follows')
                  .select('follower_id, following_id')
                  .inFilter('following_id', memberIds),
      ]);

      final likeRows = socialResults[0] as List;
      final commentRows = socialResults[1] as List;
      final myFollowRows = socialResults[2] as List;
      final memberFollowerRows = socialResults[3] as List;
      ...
      await Future.wait<void>(
        mappedPosts.map((post) async {
          ...
          final artworkPath =
              post['music_artwork_path']?.toString().trim() ?? '';
      ...
      for (final raw in memberFollowerRows) {
        final row = Map<String, dynamic>.from(raw as Map);
        final followingId = row['following_id']?.toString() ?? '';
  ...
  Widget _paginationFooter() {
    ...
    if (loadingMore) {
      return const Padding(
        ...
        child: Center(
          child: CircularProgressIndicator(color: Color(0xFF35E8FF)),
        ),
      );
    }
      ...
      child: Center(
        child: OutlinedButton.icon(
          ...
          style: OutlinedButton.styleFrom(
            foregroundColor: const Color(0xFF39E7FF),
            side: const BorderSide(color: Color(0xFF315D8A)),
        ...
        children: [
          ...
          _communityComposerIcon(
            ...
            () async {
              ...
              if (!mounted || pendingImage == null) return;

              await _openMobileComposer();
            },
          ),
          _communityComposerIcon(
  ...
  Future<void> _openStoryViewer(Map<String, dynamic> story) async {
    ...
    final authorStories = storyPosts
        .where((item) => item["author_id"]?.toString() == authorId)
        .map((item) => Map<String, dynamic>.from(item))
        .toList()
        .reversed
        .toList();

    var initialIndex = authorStories.indexWhere(
      (item) => item["id"]?.toString() == story["id"]?.toString(),
    );
    ...
    await Navigator.of(context).push(
      MaterialPageRoute(
        fullscreenDialog: true,
        builder: (_) => _CommunityStoryViewer(
  ...
  Future<void> _openReelViewer(Map<String, dynamic> reel) async {
    if (reelPosts.isEmpty) return;

    final reelId = reel['id']?.toString() ?? '';

    var initialIndex = reelPosts.indexWhere(
      (item) => item['id']?.toString() == reelId,
    );
  ...
  String _socialTimeAgo(dynamic raw) {
    if (raw == null) return '';
  ...
  Widget _storiesAndReelsSection() {
    ...
    final otherStories = storyPosts
        .where((story) => story['author_id']?.toString() != currentUserId)
        .toList();
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(
          ...
          colors: [Color(0xFF07172E), Color(0xFF071226), Color(0xFF100B27)],
        ),
        border: Border.all(
          color: const Color(0xFF22527D).withValues(alpha: .80),
        ),
        boxShadow: [
          BoxShadow(
            color: const Color(0xFF20DFFF).withValues(alpha: .07),
            blurRadius: 24,
          ),
        ],
      ),
              ...
              children: [
                Expanded(
                  child: _storiesReelsTab(
                    label: 'Stories',
                    selected: !showingReels,
                    color: const Color(0xFF29E9FF),
                    onTap: () {
                      if (!showingReels) return;
                      setState(() => showingReels = false);
                    },
                  ),
                ),
                Expanded(
                  child: _storiesReelsTab(
                    label: 'Reels',
                    selected: showingReels,
                    color: const Color(0xFFFF4CD3),
                    onTap: () {
                      if (showingReels) return;
                      setState(() => showingReels = true);
                    },
                  ),
                ),
              ],
          ...
          if (!showingReels)
            LayoutBuilder(
              builder: (context, constraints) {
                final itemCount = 2 + otherStories.length;

                final cardWidth = itemCount <= 3
                    ? ((constraints.maxWidth - 32) / itemCount).clamp(
                        92.0,
                        118.0,
                      )
                    : 104.0;
                ...
                return SizedBox(
                  ...
                  child: ListView(
                    ...
                    children: [
                      _createStoryCard(width: cardWidth),
                      ...
                      if (ownStory != null)
                        InkWell(
                          ...
                          child: _storyPreviewCard(
                            ...
                            ringColor: const Color(0xFF27E9FF),
                            isOwn: true,
                      ...
                      ...otherStories.map((story) {
                        final name =
                            story['author_name']
                                    ...
                                    .isNotEmpty ==
                                true
                            ? story['author_name'].toString()
                            : '999 Member';

                        final photo =
                            story['author_photo_url']?.toString().trim() ?? '';

                        final isVideo =
                            story['media_type']?.toString() == 'video';
                        ...
                        return InkWell(
                          ...
                          child: _storyPreviewCard(
                            ...
                            name: name,
                            subtitle:
                                _socialTimeAgo(story['created_at']).isEmpty
                                ? 'TAP TO VIEW'
                                : _socialTimeAgo(story['created_at']),
                            imageUrl: photo,
                            ringColor: const Color(0xFF9A59FF),
                            icon: isVideo
                                ? Icons.play_arrow_rounded
                                : Icons.auto_awesome_rounded,
                          ),
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                final itemCount = 1 + reelPosts.length;

                final cardWidth = itemCount <= 3
                    ? ((constraints.maxWidth - 24) / itemCount).clamp(
                        92.0,
                        118.0,
                      )
                    : 104.0;
                    ...
                    children: [
                      InkWell(
                        ...
                        child: _storyPreviewCard(
                          ...
                          subtitle: 'ADD VIDEO',
                          imageUrl: currentPhoto,
                          ringColor: const Color(0xFFFF4DD3),
                          isOwn: true,
                      ...
                      ...reelPosts.map((reel) {
                        final name =
                            reel['author_name']?.toString().trim().isNotEmpty ==
                                true
                            ? reel['author_name'].toString()
                            : '999 Member';

                        final photo =
                            reel['author_photo_url']?.toString().trim() ?? '';
                        ...
                        return InkWell(
                          ...
                          child: _storyPreviewCard(
                            width: cardWidth,
                            name: name,
                            subtitle: _socialTimeAgo(reel['created_at']).isEmpty
                                ? 'TAP TO WATCH'
                                : _socialTimeAgo(reel['created_at']),
                            imageUrl: photo,
                            ringColor: const Color(0xFFFF4DD3),
  ...
  Widget _createStoryCard({double width = 104}) {
    return InkWell(
      onTap: _openStoryCreator,
      borderRadius: BorderRadius.circular(22),
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: const LinearGradient(
            ...
            colors: [Color(0xFF21E9FF), Color(0xFF6357FF), Color(0xFFFF47D2)],
          ),
          boxShadow: [
            BoxShadow(
              color: const Color(0xFF6D55FF).withValues(alpha: .20),
              blurRadius: 16,
            ),
          ],
        ),
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            gradient: const LinearGradient(
              ...
              colors: [Color(0xFF102650), Color(0xFF101B42), Color(0xFF160D35)],
            ),
          ),
          child: Column(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: const Color(0xFF40E6FF).withValues(alpha: .26),
                      blurRadius: 14,
                    ),
                  ],
                ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          ...
          colors: [ringColor, const Color(0xFF6655FF), const Color(0xFFFF48CD)],
        ),
        boxShadow: [
          BoxShadow(color: ringColor.withValues(alpha: .15), blurRadius: 14),
        ],
      ),
      padding: const EdgeInsets.all(1.4),
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: const LinearGradient(
            ...
            colors: [Color(0xFF122B59), Color(0xFF111D48), Color(0xFF160E38)],
          ),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: LinearGradient(
                  colors: [
                    ringColor,
                    const Color(0xFF6D58FF),
                    const Color(0xFFFF4DCC),
                  ],
                ),
                boxShadow: [
                  BoxShadow(
                    color: ringColor.withValues(alpha: .30),
                    blurRadius: 10,
                  ),
                ],
              ),
  ...
  Widget _sportsMarketsPanel() {
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text(
                      label,
                      style: TextStyle(
                        color: selected ? accent : const Color(0xFFC3C4D7),
                        fontSize: 16,
                        fontWeight: FontWeight.w900,
                      ),
                    ),
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: selected
                        ? LinearGradient(
                            colors: [
                              accent,
                              const Color(0xFF7451FF),
                              const Color(0xFFFF4BCF),
                            ],
                          )
                        : null,
                    color: selected ? null : const Color(0xFF223451),
                  ),
                ),
              ],
            ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(24),
        gradient: const LinearGradient(
          colors: [Color(0xFF06162F), Color(0xFF0A1638), Color(0xFF130D32)],
        ),
        border: Border.all(
          color: const Color(0xFF2C69A4).withValues(alpha: .58),
        ),
      ),
      child: Column(
        children: [
          Row(
            children: [
              tab(
                ...
                selected: !_showMarketsPanel,
                accent: const Color(0xFF2CE9FF),
                onTap: () {
                  if (_showMarketsPanel) {
                    setState(() => _showMarketsPanel = false);
                  }
                },
              ),
              tab(
                ...
                selected: _showMarketsPanel,
                accent: const Color(0xFFFF4CCE),
                onTap: () {
                  if (!_showMarketsPanel) {
                    setState(() => _showMarketsPanel = true);
                  }
                },
              ),
            ],
          ...
          Row(
            children: [
              Icon(
                ...
                color: _showMarketsPanel
                    ? const Color(0xFFFF4CCE)
                    : const Color(0xFF35E8FF),
                size: 21,
              ),
              const SizedBox(width: 7),
              Expanded(
              ...
              Text(
                _showMarketsPanel ? 'MARKETS' : 'SCORES',
                style: TextStyle(
                  color: _showMarketsPanel
                      ? const Color(0xFFFF5BD4)
                      : const Color(0xFF8EADFF),
                  fontSize: 8,
                  fontWeight: FontWeight.w900,
                ),
              ),
  ...
  Future<String?> _mediaCaption({required bool story}) async {
    ...
    final accent = story ? const Color(0xFF39E7FF) : const Color(0xFFFF4DCE);
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: LinearGradient(
                    ...
                    colors: [
                      const Color(0xFF27E9FF),
                      const Color(0xFF7554FF),
                      accent,
                    ],
                  ),
                  boxShadow: [
                    BoxShadow(
                      color: const Color(0xFF27E9FF).withValues(alpha: .20),
                      blurRadius: 34,
                      spreadRadius: 1,
                    ),
                    BoxShadow(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              gradient: LinearGradient(
                                colors: [
                                  const Color(0xFF24E8FF),
                                  const Color(0xFF7455FF),
                                  accent,
                                ],
                              ),
                              boxShadow: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  story ? 'STORY STUDIO' : 'REEL STUDIO',
                                  style: TextStyle(
                                    color: accent,
                                    fontSize: 9,
                                    fontWeight: FontWeight.w900,
                          ...
                          InkWell(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                shape: BoxShape.circle,
                                color: const Color(0xFF101E3B),
                                border: Border.all(
                                  color: const Color(0xFF415B8F)
                                      .withValues(alpha: .55),
                                ),
                              ),
                              child: const Icon(
                      ...
                      Text(
                        story
                            ? 'Add the final words to your moment.'
                            : 'Add a caption before publishing your Reel.',
                        style: const TextStyle(
                      ...
                      Container(
                        decoration: BoxDecoration(
                          ...
                          gradient: LinearGradient(
                            colors: [
                              const Color(0xFF0D2348),
                              accent.withValues(alpha: .08),
                              const Color(0xFF161238),
                            ],
                          ),
                          border: Border.all(
                      ...
                      SizedBox(
                        ...
                        child: DecoratedBox(
                          ...
                          child: FilledButton.icon(
                            ...
                            label: Text(
                              story ? 'Share Story' : 'Publish Reel',
                              style: const TextStyle(
                                color: Colors.white,
                                fontSize: 13,
                                fontWeight: FontWeight.w900,
      ...
      child: _glassPanel(
        ...
        child: Column(
          ...
          children: [
            ...
            if (hasMusic && !hasPhoto) _musicFeedCard(post),
            if (media.isNotEmpty)
              Padding(
                          ...
                          Expanded(
                            ...
                            child: Column(
                              children: [
                                ...
                                if (selectedTab != 2 && feedPosts.isNotEmpty)
                                  _paginationFooter(),
                              ],
                            ),
                          ),
        ...
        return Column(
          ...
          children: [
            ...
            if (loading)
              ...
              for (final post in feedPosts) _postCard(post),
            if (selectedTab != 2 && feedPosts.isNotEmpty) _paginationFooter(),
          ],
        );
      },
    );
  ...
  Widget _mobileCompetitionCategories() {
    const items = [
      ('Sports', Icons.sports_basketball_rounded),
      ('Wellness', Icons.spa_rounded),
      ('Mindset', Icons.psychology_alt_rounded),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color(0xFF06132B),
        borderRadius: BorderRadius.circular(22),
        border: Border.all(
          color: const Color(0xFF27689C).withValues(alpha: .62),
        ),
        boxShadow: [
          BoxShadow(
            color: const Color(0xFF28DFFF).withValues(alpha: .06),
            blurRadius: 18,
          ),
        ],
      ),
      child: SingleChildScrollView(
        ...
        child: Row(
          children: List.generate(items.length, (index) {
            ...
            final selected = _mobileSocialCategory == index;

            final accent = index == 0
                ? const Color(0xFF2DEAFF)
                : index == 1
                ? const Color(0xFF55E8C5)
                : index == 2
                ? const Color(0xFF9B69FF)
                : index == 3
            ...
            return Padding(
              ...
              child: Material(
                ...
                child: InkWell(
                  ...
                  onTap: () {
                    setState(() {
                      _mobileSocialCategory = index;
                    });

                    if (index == 0) {
                      widget.onSports?.call();
                    }
                  },
                  child: AnimatedContainer(
                    duration: const Duration(milliseconds: 180),
                    ...
                    decoration: BoxDecoration(
                      ...
                      gradient: selected
                          ? LinearGradient(
                              colors: [
                                accent.withValues(alpha: .26),
                                const Color(0xFF3049B0).withValues(alpha: .18),
                              ],
                            )
                          : null,
                      border: selected
                          ? Border.all(color: accent.withValues(alpha: .78))
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(
                          item.$2,
                          color: selected ? accent : const Color(0xFFAAB7CE),
                          size: 17,
                        ),
                        ...
                        Text(
                          item.$1,
                          style: TextStyle(
                            color: selected ? accent : const Color(0xFFC0C9DA),
                            fontSize: 11,
                            fontWeight: selected
                                ? FontWeight.w900
                                : FontWeight.w700,
...
class _CommunityStoryViewer extends StatefulWidget {
  ...
  @override
  State<_CommunityStoryViewer> createState() => _CommunityStoryViewerState();
}
...
class _CommunityStoryViewerState extends State<_CommunityStoryViewer> {
  ...
  bool get _isVideo => _story['media_type']?.toString() == 'video';

  String get _mediaUrl => _story['_media_url']?.toString().trim() ?? '';
  ...
  Future<void> _initializeVideo() async {
    try {
      ...
      controller.addListener(_storyVideoListener);
  ...
  Future<void> _switchStory(int index) async {
    final oldController = _videoController;

    if (oldController != null) {
      oldController.removeListener(_storyVideoListener);
  ...
  Future<void> _nextStory() async {
    if (_currentIndex < widget.stories.length - 1) {
      await _switchStory(_currentIndex + 1);
      return;
    }
  ...
  String get _storyAuthorId => _story['author_id']?.toString().trim() ?? '';

  bool get _isOwnStory =>
      Supabase.instance.client.auth.currentUser?.id == _storyAuthorId;

  Future<void> _sendStoryReply([String? reaction]) async {
    if (_sendingStoryReply || _isOwnStory) return;

    final user = Supabase.instance.client.auth.currentUser;
    if (user == null || _storyAuthorId.isEmpty) return;

    final body = (reaction ?? _storyReplyController.text).trim();
    if (body.isEmpty) return;

    setState(() {
      _sendingStoryReply = true;
    });

    try {
      await Supabase.instance.client.from('messages').insert({
        'sender_id': user.id,
        'receiver_id': _storyAuthorId,
        'message': body,
      });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Unable to send Story reply: $error')),
      );
    } finally {
      if (mounted) {
        setState(() {
          _sendingStoryReply = false;
        });
      }
    }
  }
              ...
              child: InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(18),
                    color: destructive
                        ? const Color(0xFFFF315D).withValues(alpha: .09)
                        : const Color(0xFF0B1B3B).withValues(alpha: .88),
                    border: Border.all(
                      color: destructive
                          ? const Color(0xFFFF5578).withValues(alpha: .42)
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: const LinearGradient(
              ...
              colors: [Color(0xFF20E8FF), Color(0xFF5558FF), Color(0xFFB63DFF)],
            ),
            boxShadow: [
              BoxShadow(
                color: const Color(0xFF20E8FF).withValues(alpha: .22),
                blurRadius: 30,
              ),
              BoxShadow(
                color: const Color(0xFFA73EFF).withValues(alpha: .20),
                blurRadius: 30,
              ),
            ],
          ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    actionTile(
                      ...
                      title: _muted ? 'Turn Sound On' : 'Mute Story',
                      subtitle: 'Control story audio',
                    ...
                    actionTile(
                      ...
                      title: 'Restart Story',
                      subtitle: 'Play this story from the beginning',
                    ...
                    actionTile(
                      ...
                      subtitle: 'Go back to the previous story',
                      color: const Color(0xFF38E7C5),
                      onTap: () {
                        Navigator.of(sheetContext).pop();
                        _previousStory();
                      },
                    ...
                    actionTile(
                      ...
                      title: 'Next Story',
                      subtitle: 'Continue to the next story',
                      color: const Color(0xFFFF55CC),
                      onTap: () {
                        Navigator.of(sheetContext).pop();
                        _nextStory();
                      },
                    ...
                    if (isOwner)
                      actionTile(
                        ...
                        title: 'Delete Story',
                        subtitle: 'Permanently remove this story',
                        ...
                        onTap: () async {
                          ...
                          final storyId = _story['id']?.toString().trim() ?? '';
                          ...
                          try {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('Unable to delete story: $error'),
                              ),
                            );
                          }
                        },
                    ...
                    actionTile(
                      ...
                      title: 'Close Story',
                      subtitle: 'Return to your community feed',
                      color: const Color(0xFF9AA7C8),
                      onTap: () {
                        Navigator.of(sheetContext).pop();
  ...
  @override
  void dispose() {
    _videoController?.removeListener(_storyVideoListener);
  ...
  @override
  Widget build(BuildContext context) {
    final author = _story['author_name']?.toString().trim() ?? '999 Member';

    final photo = _story['author_photo_url']?.toString().trim() ?? '';

    final caption = _story['body']?.toString().trim() ?? '';
    ...
    if (_mediaUrl.isEmpty) {
      ...
      if (_videoError != null) {
        ...
        media = LayoutBuilder(
          builder: (context, constraints) {
            final videoWidth = size.width <= 0 ? 9.0 : size.width;
            final videoHeight = size.height <= 0 ? 16.0 : size.height;
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'your story - ${_currentIndex + 1} of ${widget.stories.length}',
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 13,
                            fontWeight: FontWeight.w800,
              ...
              Positioned(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 7),
                    GestureDetector(
                      onTap: () => _sendStoryReply('❤️'),
                      child: const CircleAvatar(
                        radius: 23,
                        backgroundColor: Color(0xFFFF295E),
                        child: Text('❤️', style: TextStyle(fontSize: 20)),
                      ),
                    ),
                    const SizedBox(width: 6),
                    GestureDetector(
                      onTap: () => _sendStoryReply('👍'),
                      child: const CircleAvatar(
                        radius: 23,
                        backgroundColor: Color(0xFF2586FF),
