...
  @override
  State<_CommunityStoryViewer> createState() =>
...
  String get _authorId => _story['author_id']?.toString() ?? '';

  bool get _isOwnStory =>
      Supabase.instance.client.auth.currentUser?.id == _authorId;
...
      _index = 0;
    } else if (widget.initialIndex >= widget.stories.length) {
...
    if (!_isVideo || _mediaUrl.isEmpty) {
      Future<void>.delayed(const Duration(seconds: 5), () {
        if (!mounted || _isVideo || _advancing) return;
...
    if (duration > 0) {
      final nextProgress = (position / duration).clamp(0.0, 1.0);

      if ((nextProgress - _progress).abs() > .01) {
        setState(() {
...
  Future<void> _goToStory(int nextIndex) async {
...
    if (_sendingReply || _isOwnStory) return;

    final user = Supabase.instance.client.auth.currentUser;

    if (user == null || _authorId.isEmpty) return;

    final message = (reaction ?? _replyController.text).trim();
...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Unable to send Story reply.'),
        ),
      );
    } finally {
      if (mounted) {
...
  Widget build(BuildContext context) {
    final author =
        _story['author_name']?.toString().trim() ?? '999 Member';

    final photo =
        _story['author_photo_url']?.toString().trim() ?? '';
